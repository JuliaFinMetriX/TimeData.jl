...
resulting values will automatically be embed into the metadata again.
Applying a function iteratively and simply capturing the results in an
~Array{Any, 1}~ is a quite easy task with comprehensions. However,
automatically storing the data in a meaningful way encapsulated by its
respective metadata is the hard part of the task. This, however, can
be easily achieved through a combination of iterators and ~map~ for a
quite general set tasks.
...
different standard iterators, each return different slices of a given
~TimeData~ object:
